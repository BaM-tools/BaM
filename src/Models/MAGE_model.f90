...
subroutine MAGE_Run(exeFile,version,projectDir,REPfile,theta,Y,feas,err,mess)
!^**********************************************************************
!^* Purpose: Run MAGE
!^**********************************************************************
!^* Programmer: Ben Renard and Felipe Mendez, INRAE Aix & INRAE Lyon
...
do i=1,size(outFiles)
    ...
    case('8','v8')
        call ReadSeparatedFile(file=outFiles(i),sep=';',nhead=1,ncol=outFileNCOL,y=foo,err=err,mess=mess)
    case default
        if(err/=0) then;mess=trim(procname)//':FATAL: MAGE version is not supported';return;endif
    end select
    if(err/=0) then;mess=trim(procname)//':'//trim(mess);return;endif
...
read(unt,*,iostat=err) xtra%cp1(1) ! 5. Z file (covariates for Kmin-Z regression)
if(err/=0) then;mess=trim(procname)//':problem reading file '//trim(file);return;endif
...
if(err/=0) then;mess=trim(procname)//':problem reading file '//trim(file);return;endif
read(unt,*,iostat=err) xtra%cp1(2) ! 8. Z file (covariates for Kmoy-Z regression)
if(err/=0) then;mess=trim(procname)//':problem reading file '//trim(file);return;endif
...
subroutine MAGE_setUp(version,projectDir,REPfile,Zfile,ZnCol,doExp,err,mess)
!^**********************************************************************
