...
err=0;mess=''
npar=size(RUGb)*2 ! For each reach, one parameter for main chanel and one for floodplain
...
write(unt,'(A)') '*---======----------==========----------=========='
do i=1,n
    write(unt,forma) 'K',RUGb(i),RUGx(i),RUGx(i+1),theta(2*i-1),theta(2*i)
enddo
close(unt)
...
subroutine MAGE_setUp(projectDir,REPfile,err,mess)
!^**********************************************************************
...
character(*), intent(in)::projectDir,REPfile
integer(mik), intent(out)::err
character(*),intent(out)::mess
!locals
character(250),parameter::procname='MAGE_setUp'
integer(mik)::unt,i,n,k
character(250)::foo,prefix,var,ib,pk,mode,dt0,forma,bar
